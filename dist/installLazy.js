...
export const installLazy = async () => {
    const lazyPath = path.join(getHomeDirectory(), '.local', 'share', 'nvim', 'lazy', 'lazy.nvim');
    ...
    try {
        await execa('git', ['clone', '--filter=blob:none', '--branch=stable', LAZY_REPO, lazyPath], {
            stdio: 'inherit',
        });
        ...
        console.warn('Could not pre-install lazy.nvim - Neovim will bootstrap it on first launch.', error);
    }
};
